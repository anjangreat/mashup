...
def download_videos(singer_name, num_videos, save_dir):
    ...
    for idx, video_url in enumerate(video_urls):
        try:
            ...
            if stream:
                video_file = os.path.join(save_dir, f"{singer_name}_video_{idx + 1}.mp4")
...
def main():
    ...
    if num_videos < 1 or num_videos > 10:
        print("Number of videos must be between 1 and 10.")
        sys.exit(1)
    ...
    save_dir = f"{singer_name}_downloads"
    create_directory(save_dir)
    video_paths = download_videos(singer_name, num_videos, save_dir)
    if not video_paths:
